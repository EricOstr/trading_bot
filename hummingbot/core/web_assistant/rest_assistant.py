...
class RESTAssistant:
    ...
    async def execute_request(
        ...
        # used to indicate the media type of the sent request body
        # x-www-form-urlencoded is a media type that is used to send data in the HTTP request body.
        local_headers = { 
        ...
        # in this async context manager the throttler checks for viability of request against RateLimits
    ...
    async def call(self, request: RESTRequest, timeout: Optional[float] = None) -> RESTResponse:
        '''
        If we already have a well packaged RESTRequest in the outside, we 
        can directly call this (as is the case in CoinbaseProExchange _api_req
        '''
        request = deepcopy(request)
        request = await self._pre_process_request(request)
        request = await self._authenticate(request)
        resp = await wait_for(self._connection.call(request), timeout) # resp is a RESTResponse
