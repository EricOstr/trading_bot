...
from hummingbot.core.cpp.order_book_entry import OrderBookEntry, truncateOverlapEntries
...
class OrderBook(PubSub):
    ORDER_BOOK_TRADE_EVENT_TAG = OrderBookEvent.TradeEvent.value # 901
    ...
    # def c_apply_diffs(self, bids : List[OrderBookEntry], asks: List[OrderBookEntry], update_id: int):
    ...
    def c_apply_snapshot(self, bids: List[OrderBookEntry], asks: List[OrderBookEntry], update_id: int):
        ...
        self._bid_book = bids[:] # creates a new list object and copies all the elements
        self._ask_book = asks[:]
        ...
        best_bid_price = max(self._bid_book) or float("NaN")
        best_ask_price = min(self._bid_book) or float("NaN")

        # if self._dex:
        #     truncateOverlapEntries(self._bid_book, self._ask_book, self._dex)
        #     self._best_bid = self._bid_book[-1].price if len(self._bid_book) > 0 else None
        #     self._best_ask = self._ask_book[0].price if len(self._ask_book) > 0 else None
    ...
    def apply_snapshot(self, bids: List[OrderBookRow], asks: List[OrderBookRow], update_id: int):
        ...
        for row in bids:
            cpp_bids.append(OrderBookEntry(row.price, row.amount, row.update_id))
        for row in asks:
            cpp_asks.append(OrderBookEntry(row.price, row.amount, row.update_id))
