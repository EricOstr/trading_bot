...
# def class CoinbaseProExchangeTransactionTracker(TransactionTracker):
    # def:
...
class CoinbaseProExchange(ExchangeBase):
# class CoinbaseProExchange():
    ...
    def __init__(self,
                 client_config_map: "ClientConfigAdapter", # do something about this
    ...
    async def start_network(self):
        ...
        *required
        ...
        await self.order_book_tracker.start() # debug        
    ...
    def _stop_network(self):
        ...
        """
        self.order_book_tracker.stop() # calls .cancel() on all the tasks in OrderBookTracker
    ...
    async def execute_buy(self,
        ...
        decimal_amount = amount # replaces the code above
    ...
    def c_buy(self, trading_pair, amount, order_type=OrderType.LIMIT, price=s_decimal_0,
                    kwargs={}):
        """
        *required
        ...
        # res = safe_ensure_future(self.execute_buy(order_id, trading_pair, amount, order_type, price))
        asyncio.get_event_loop().run_until_complete(self.execute_buy(order_id, trading_pair, amount, order_type, price))
        return order_id
    ...
    def c_sell(self,
        ...
        *required
        ...
        asyncio.get_event_loop().run_until_complete(self.execute_sell(order_id, trading_pair, amount, order_type, price))
        return order_id
    ...
    def c_cancel(self, trading_pair: str, order_id: str):
        """
        *required
        Synchronous wrapper that schedules cancelling an order.
        """
        # res = safe_ensure_future(self.execute_cancel(trading_pair, order_id))
        # asyncio.get_event_loop().run_until_complete(res)
        # res = asyncio.run(self.execute_cancel(trading_pair, order_id))
        asyncio.get_event_loop().run_until_complete(self.execute_cancel(trading_pair, order_id))
        return order_id
    ...
    # def object c_quantize_order_amount(self, str trading_pair, object amount, object price=s_decimal_0):
    #     """
    #     *required
