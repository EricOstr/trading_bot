...
#             raise ValueError(f"Unknown message type '{msg_type}' - {message}. Aborting.")
    ...
    def c_convert_snapshot_message_to_np_arrays(self, message: "CoinbaseProOrderBookMessage"):
        """
        Interprets a CoinbaseProOrderBookMessage
        :returns: new order book rows: Tuple(np.array (bids), np.array (asks))
        """
        ...
        bids = np.array(
            [[message.timestamp,
                ...
                message.update_id]
                for price in sorted(self._active_bids.keys(), reverse=True)], dtype="float64", ndmin=2) # sort in desc order
